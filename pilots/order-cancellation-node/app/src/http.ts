...
import { ApplicationError, OrderCancellationService } from "./service.ts";
...
export function createRequestHandler(options: ApplicationOptions) {
  const logger = options.logger ?? ((event: LogEvent) => console.log(JSON.stringify(event)));
    ...
    try {
      ...
      if (method === "GET") {
        ...
        if (contextMatch) {
          ...
          sendJson(response, 200, options.service.getContext(actorId, contextMatch[1]!));
          return;
        }
      }

      if (method === "POST") {
        const commandMatch = url.pathname.match(/^\/orders\/([^/]+)\/cancellations$/);
        if (commandMatch) {
          ...
          const receipt = options.service.requestCancellation(commandInput);
          response.setHeader("Location", `/order-cancellations/${receipt.cancellationId}`);
      ...
      if (method === "GET") {
        const statusMatch = url.pathname.match(/^\/order-cancellations\/([^/]+)$/);
        if (statusMatch) {
          ...
          sendJson(
            response,
            200,
            options.service.getCancellation(actorId, statusMatch[1]!),
          );
          return;
        }
      }

      if (method === "GET" && (await serveStatic(url.pathname, options.publicDirectory, response))) {
        route = "static";
        return;
      }
      ...
      if (error instanceof ApplicationError) {
        sendJson(response, error.statusCode, applicationErrorBody(error, traceId));
      } else {
